...
//	private static Semaphore userOutputMutex = new Semaphore(1);
	...
	public static void main(String[] args) throws InterruptedException, IOException {
		...
		System.out.println("Please enter the Arrival time for Program_1:");
		scanner = new Scanner(System.in);
		Program1start = scanner.nextInt();
		System.out.println("Please enter the Arrival time for Program_2:");
		scanner = new Scanner(System.in);
		Program2start = scanner.nextInt();
		System.out.println("Please enter the Arrival time for Program_3:");
		scanner = new Scanner(System.in);
...
//					}
		...
		if (Program1start == currenttime) {
			System.out.println(Process1.getName() + " arrived");
		...
		if (Program2start == currenttime) {
			System.out.println(Process2.getName() + " arrived");
		...
		if (Program3start == currenttime) {
			System.out.println(Process3.getName() + " arrived");
	...
	public static void schedule() throws IOException, InterruptedException {
		...
		while ((!readyQueue.isEmpty() || !blockedQueue.isEmpty())
			...
			System.out.println(currentProcess.toStringg());
...
//				System.out.println("currenttime ="+currenttime);
			...
			for (int i = currentProcess.getPcb().getMemoryBoundaries()[0]
					+ 4; i < currentProcess.getPcb().getMemoryBoundaries()[1] - 2; i++) {
				if (memory[currentProcess.getPcb().getProgramCounter()] != null) {
//						System.out.println(memory[currentProcess.getPcb().getProgramCounter()]);
				...
				if (parts[2].equals("readFile")) {
					...
					for (firstloc = currentProcess.getPcb().getMemoryBoundaries()[1] - 2; i1 <= 3
						...
						if (((String) varii[0]).equals(from)) {
							...
							System.out.println("there is no value assigned to variable " + from);
						}
						firstloc++;
					}
					String data1 = "";
					try (BufferedReader newreader = new BufferedReader(
							new FileReader("src/resources/" + value + ".txt"))) {
						String line = "";
...
//						}
			...
			case "writeFile":
				...
				for (firstloc = currentProcess.getPcb().getMemoryBoundaries()[1] - 2; i1 <= 3
					...
					if (((String) varii[0]).equals(fileName)) {
						...
						System.out.println("there is no value assigned to variable " + fileName);
					}
					firstloc++;
				}
				if (i1 < 3 && memory[firstloc] == null) {
				...
				for (firstloc = currentProcess.getPcb().getMemoryBoundaries()[1] - 2; i1 <= 3
					...
					if (((String) varii[0]).equals(data)) {
						...
						System.out.println("there is no value assigned to variable " + data);
					}
					firstloc++;
				}
				if (i1 < 3 && memory[firstloc] == null) {
			...
			case "printFromTo":
				...
				for (firstloc11 = currentProcess.getPcb().getMemoryBoundaries()[1] - 2; i111 <= 3
					...
					if (varii[0].equals(to)) {
						...
						System.out.println("there is no value assigned to variable " + to);
					}
					firstloc11++;
				}
	...
	public static void putintodisk(Process hello) throws IOException {
		...
		System.out.println("Process " + hello.getPcb().getProcessID() + " was added to disk");
	}
...
//										System.out.println("boundddd 1 " + tobeswapped.getPcb().getMemoryBoundaries());
...
//											System.out.println(m + " " + memory[m]);
							...
							System.out.println("Process " + swappedwith.getPcb().getProcessID() + " and Process "
				...
				System.out.println("Program file not found: disk");
			}
		}
	}
	...
	public static void getfromdisk(Process getitt) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader("src/OSclasses/disk.txt"))) {
			...
			writer.close();
			System.out.println("Process " + getitt.getPcb().getProcessID() + " was added to memory");
		} catch (IOException e) {
			System.out.println("Program file not found: disk");
		}
